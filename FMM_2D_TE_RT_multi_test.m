function [eta_R, eta_T, gamma, R_T, ud_PMM, W] = FMM_2D_TE_RT_multi_test(eps11,eps22,eps33,...
    periodx, periody, h, lambda, theta, phi, refIndices, N, M, L)
%
% INPUT:
...
for i=1:L
    [Wt, pplust, pminust, gammat] = FMM_2D_gamma_e_test(eps11(:,:,i),eps22(:,:,i),eps33(:,:,i),alpha, beta, h(i),lambda,N);
...
FMM_graph_output(R_T, alpha0, beta0, N, periodx, periody);

f_coef = PMM_to_FMM_RT_inverse(N,NN);

R1_old = R_T(1:NN);
R2_old = R_T((NN+1):2*NN);
T1 = R_T((2*NN+1):3*NN);  %d0
T2 = R_T((3*NN+1):4*NN);

u1_PMM = f_coef*R1_old;
u2_PMM = f_coef*R2_old;
d1_PMM = f_coef*T1;
d2_PMM = f_coef*T2;
ud_PMM = cat(1,u1_PMM,u2_PMM,d1_PMM,d2_PMM);

R1 = zeros(NN,1);
R2 = zeros(NN,1);
for i=1:NN
    R1(i) = R_T(i)/exp(1j*kz1v(i)*(sum(h)-h(L)) );
    R2(i) = R_T(i+NN)/exp(1j*kz1v(i)*(sum(h)-h(L)) );
end


%R1 = R_T(1:NN)/exp( 1j*gamma01*(sum(h)-h(L)) );        %u_last
%R2 = R_T((NN+1):2*NN)/exp( 1j*gamma01*(sum(h)-h(L)) );
%!!!!shouldn't we change gamma01 to different values kz1v(m)? check out!
