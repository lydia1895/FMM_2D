...
h(1) = 20*10^(-9);
M = 251;               %number of modes for Fourier transform of epsilon
Mrx = (rx/a)*M;
Mry = (ry/a)*M;

%for gradient space at the ellipse boundary
alph0 = -0.5;
n0=11;
for n = 1:(n0+1)
    alph(n) = alph0+ (n-1)/n0
end
delnm = 10*(10^(-9)); %gap width in nm
delta = delnm*(M/a);
...
for k =1:Nl    
    ...
    if ( ((i-i0)^2/(Mrx-delta/2)^2+(j-j0)^2/(Mry-delta/2)^2) < 1)
        epsilon(j,i,1,k) = nAu(k)^2;
    else
        epsilon(j,i,1,k) = nZep(k)^2;
    end
    for n=2:n0
        inner_radius = (i-i0)^2/(Mrx+alph(n-1)*delta)^2 + (j-j0)^2/(Mry+alph(n-1)*delta)^2;
        outer_radius = (i-i0)^2/(Mrx+alph(n)*delta)^2 + (j-j0)^2/(Mry+alph(n)*delta)^2;
        if (inner_radius >= 1) && (outer_radius < 1)
            epsAu = nAu(k)^2;
            epsZep = nZep(k)^2;
            epsilon(j,i,1,k) = epsAu + (epsZep-epsAu)*(delta/2+alph(n)*delta)/delta;
        end
    end
