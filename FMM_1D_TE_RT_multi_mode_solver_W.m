...
K1_new = cat(2,K1(:,2*NN+1:4*NN),K1(:,1:2*NN));
K2_new = cat(2,K2(:,2*NN+1:4*NN),K2(:,1:2*NN));


Smin1 = eye(4*NN,4*NN);
S0 = new_recursion(Smin1, K2_new, W(:,:,1), eye(2*NN,2*NN), pminus(:,:,1), N);
Stemp = S0;
if L>1
    for i=1:(L-1)
...
Rtotal = new_recursion(Stemp, W(:,:,L), K1_new, pplus(:,:,L), eye(2*NN,2*NN), N);
