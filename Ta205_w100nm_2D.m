...
epsSiO2 = nSiO2^2;
nSi = 3.71;
epsilon(:,:,1)=epsSiO2*ones(M,M);
epsilon(:,:,2)=epsTa2O5*ones(M,M);
%epsilon(:,:,3)=epsTa2O5*ones(M,M);
for i=1:M
    for j=1:M
    if x(i)<dx
        epsilon(j,i,2) = 1.0;
...
figure(5)
hold on
plot(lambda, Rsum(:,1), 'b', lambda, Rsum(:,2)+0.5, 'g', lambda, Rsum(:,3)+1, 'r', 'LineWidth', 2)
%h5 = legend('theta=0','theta=1','theta=2','theta=3',4);
h5 = legend('theta=0.2','theta=0.4','theta=0.6',4);
set(h5,'Interpreter','none')
%title('W=',w*10^9,' nm, D=', period*10^9,' nm')
axis tight
axis([lmin lmax 0 2.0])
set(gca,'fontsize', 18)
hold off
%{
figure(5)
