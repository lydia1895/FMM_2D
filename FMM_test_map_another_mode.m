...
tmin = 0;
tmax = 60;
lambda = linspace(lmin,lmax,35)*10^(-9);
lambdanm = linspace(lmin,lmax,35);
[Nll, Nl] = size(lambda)
...
for k=1:Nl
    ...
    epsilon(:,:,2,k) = ones(M,M);
    epsilon(:,:,1,k) = nSiO2(k)*ones(M,M);
    for i=1:(i0-1)
    for j=1:(j0-1)         
        if ( ((i-i0)^2/(Mrx^2)+(j-j0)^2/(Mry^2)) <= 1)    
        epsilon(j,i,2,k) = nAu(k)^2;
        epsilon(M-j+1,i,2,k) = nAu(k)^2;
        epsilon(j,M-i+1,2,k) = nAu(k)^2;
        epsilon(M-j+1,M-i+1,2,k) = nAu(k)^2;
        end
    end
    end
    i = i0;
